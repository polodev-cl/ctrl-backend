...
export class CompanyService {
  ...
  public async list(queryParams?: CompanyQueryDto) {
    const whereFilter: FindOptionsWhere<CompanyEntity> = Object.keys(
      queryParams
    ).reduce((acc, key) => {
      if (queryParams[key]) {
    ...
    if (queryParams.prestador !== undefined)
      whereFilter['prestador'] = Equal(queryParams.prestador);
  ...
  public async update(id: number, updateCompanyDto: CreateCompanyDto) {
